...
arctic_ice[which(arctic_ice$Year == 1979),'Extent'] # Find the daily data for the first year
arctic_ice_annual$extent_annual_avg[1] = mean(arctic_ice[which(arctic_ice$Year == arctic_ice_annual$Year[1]),'Extent']) # calc and store mean extent for that first year (without hard coding year)
...
# 5-year average:
for (i in seq(3, dim(arctic_ice_annual)[1]-2)) # skip the first 2 and last 2 years
{
...
arctic_ice_annual$date = make_date(year=arctic_ice_annual$Year) # default month and day = 1
...
```{r}
ice_loss_million_km2 = arctic_ice_annual$extent_annual_avg[dim(arctic_ice_annual)[1]] - arctic_ice_annual$extent_annual_avg[1]
time_period = max(arctic_ice_annual$Year) - min(arctic_ice_annual$Year)
